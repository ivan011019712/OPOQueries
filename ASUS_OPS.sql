...
insert #INV
    select POVendor,MP,MatNo,Qty,INVDate,'' from Ivan_CurrentINV  where Customer='ASUS' and INVDate>=convert(char(10),getdate(),111) 
...
POQty int,DNQty int,DockQty int,ShipQty int,OpenQty int,Shortage varchar(10),PIC varchar(20),MP varchar(10),Escalation varchar(20),EscalationDate varchar(20),Remark varchar(1000),Pty int)
...
select distinct Site,PO,SO,Item,'','',rtrim(CPQPN),IECPN,ProductFamily,'',Date850,left(SO_ReqDate,4)+'/'+substring(SO_ReqDate,5,2)+'/'+substring(SO_ReqDate,7,2),'',
isnull(PO_Type,''),isnull(Model_Status,''),isnull(FCST_Status,''),'',TCDate='',NeedShipDate=SO_First_Date,
POQty=convert(int,Qty850),0,0,Ship=0,OpenQty=0,'','','','','','',9999 from Service_APD  where PO in (
select distinct PO from (
select distinct PO from ZM57 where Site  in ('ASUS-CSC','ASUS-FRU')
union
...
------(2020/09/08) 
update #init_tmp_OPO set NeedShipDate='2020/09/24'  where PO='115220000042385'

-------(2020/06/19) Add PO Priority 
update #init_tmp_OPO set Pty=b.Pty from #init_tmp_OPO a,ASUSPty$ b where a.PO=b.PO
--select * from #init_tmp_OPO a,ASUSPty$ b where a.PO=b.PO
...
--update #init_tmp_OPO set POVendor='IES' where POVendor='IPE-CP85'
--update #init_tmp_OPO set POVendor='IES' where POVendor='IPE-CP82'
--update #init_tmp_OPO set POVendor='IES' where POVendor='IPE-CP83'
update #init_tmp_OPO set POVendor='ICC' where POVendor='ICC-CP69'

----SO initial is 2 ,set POVendor='IES'
update #init_tmp_OPO set POVendor='ICC' where SO like '2%'

---(2020/07/01) Force to Add POVendor.
...
update #init_tmp_OPO set IECPO=convert(varchar(20),convert(decimal(20,0),b.IPCSO)),POItem=b.IPCSOItem,PType=b.POType from #init_tmp_OPO a,ZM57 b where a.SO=b.IECSO and a.Item=b.IECSPItem and a.IECPO='' 
...
--select * from #init_tmp_OPO where Model_Status=''
declare @x int
declare @y int
...
-------Change Date to CalendarDate
    ...
    insert #ETA
       select @dd,'5',convert(char(10),max(Dt),111) from(
       select Top 35/*25*/ DID,Dt from Calendar3 where Dt>convert(datetime,@dd+' 00:00') /*and WorkingDay='1'*/) as a 
    insert #ETA
       select @dd,'13',convert(char(10),max(Dt),111) from(
       select Top 91/*45*/ DID,Dt from Calendar3 where Dt>convert(datetime,@dd+' 00:00') /*and WorkingDay='1'*/) as a 
   
    select @x=@x+1
end

----Update IECETA
update #init_tmp_OPO set TCDate=b.ETADate from #init_tmp_OPO a,#ETA b where a.POReceiveDate=b.IssueDate  and a.Model_Status='MP' and b.ETAWeek='5'
update #init_tmp_OPO set TCDate=b.ETADate from #init_tmp_OPO a,#ETA b where a.POReceiveDate=b.IssueDate  and a.Model_Status='EOL' and b.ETAWeek='13'
...
---(2014/08/29) Remove this rule for Dell
...
------(2015/0831)先硬抓
update #init_tmp_OPO set Site='ASUS-CSC' from #init_tmp_OPO
...
--Update Escalation
update #init_tmp_OPO set Escalation=b.Escalation,EscalationDate=b.[Escalation Date],Remark=b.Remark from #init_tmp_OPO a,ASUS_OPO$ b where a.Site=b.Site and a.SO=b.SO and a.IECPN=b.IECPN
...
*/
--select * from #init_tmp_OPO order by Pty,NeedShipDate,IECPN         

insert #tmp_OPO
   --select distinct * from #init_tmp_OPO order by NeedShipDate          
   select  Site ,PO ,SO ,Item ,IECPO ,POItem ,CPQNo ,IECPN ,ProductFamily ,POVendor ,
   POReceiveDate ,SORequestDate ,CustETADate ,PO_Type ,Model_Status ,FCST_Status ,PType ,TCDate ,NeedShipDate ,
   POQty ,DNQty ,DockQty ,ShipQty ,OpenQty ,Shortage ,PIC ,MP ,Escalation ,EscalationDate ,Remark from #init_tmp_OPO order by Pty,NeedShipDate,IECPN          
 
 --select *,[Demand Type],[EBS DN No] from ASUSSHIP$
 update #tmp_OPO set Model_Status='-'
 update #tmp_OPO set Model_Status=b.[Demand Type],NeedShipDate=case when b.Memo='' then b.[Demand Date] else b.Memo end  from #tmp_OPO a,ASUSSHIP$ b where a.PO=b.[EBS DN No]



----Add IES_DN
--select * from #tmp_OPO
update #tmp_OPO set Escalation=isnull(b.IES_DN,''),EscalationDate=IES_DNPGI from #tmp_OPO a,Service_APD b where a.SO=b.SO and a.Item=b.Item and a.IECPN=b.IECPN
--------
/*
drop table #MPinit
drop table #MPmid
...
insert #MPinit
   ...
   select distinct POVendor,IECPN from #tmp_OPO where /*MP='Y' and*/ not IECPN like '6%' and POVendor='ICC') as a,t_download_org_bom_CP69DW b where a.IECPN=b.Material and left(b.Component,1) in ('1','2')
...
insert #MPinit
   ...
   #MPmid a,t_download_org_bom_CP69DW b where a.SAPN=b.Material and left(b.Component,1) in ('1','2') and POVendor='ICC'

delete #MPmid
    select @Bi=@Bi+1
end

-----(2021/07/06) manual alternative for 172 代用
update #MPresult set AG='01',Usage='50' where IECPN in(
select IECPN from(
select IECPN,qty=count(*) from #MPresult where IECPN like '%MB002' and ASPN like '172%' and AG is null group by IECPN) as a where qty>1)


-----(2021/07/21) 硬改---又有172代用又有代用
update #MPresult set AG='02',Usage='30',Item='11000' where IECPN in(
select IECPN from #MPresult where IECPN in (select IECPN from #MPresult where AG is null)  and not AG is null) and not AG is null

update #MPresult set AG='02',Usage='40',Item='11000' where IECPN in(
select IECPN from #MPresult where IECPN in (select IECPN from #MPresult where AG is null)  and not AG is null) and AG is null
/*
select * from #MPresult where IECPN in (
select IECPN from #MPresult where IECPN in (select IECPN from #MPresult where AG is null)  and not AG is null) order by IECPN
*/



/*
select * from #MPresult where IECPN='LFE033AMB002'
select * from #Mat_PF where Pno='LFE020ALT002'
select * from #result where PF='LFE033AMB002'
select * from #AAResult where PF='LFE033AMB002'
select * from #Alternative where PF='LFE033AMB002'
*/
...
where Site in (Select distinct ZS92Site from SiteMapping where Customer='ASUS' and not ZS92Site='') 
...
-----(2020/06/04) Force to find "SHIP" Qty
--select * from #INV a, #tmp_OPO_DN b where a.MatNo=b.IECPN  
update #INV set MaterialProperty='SHIP' from #INV a, #tmp_OPO_DN b where a.MatNo=b.IECPN  and a.MaterialProperty='FG'
declare @iid int
declare @DX int
declare @DY int
...
while @DX<=@DY
begin
    select @iid=iid,@POVendor=POVendor,@IECPN=IECPN,@OpenQty=OpenQty,@DNQty=DNQty from #tmp_OPO_DN where pid=@DX
    if @DNQty>0
    begin
		...
		begin
			...
			update #INV set Qty=0,Remark=rtrim(Remark)+';('+convert(varchar(5),@iid)+')-->'+convert(varchar(5),@DockQty) where POVendor=@POVendor and MatNo=@IECPN and MaterialProperty='SHIP'
			end
			else
			begin
			update #tmp_OPO_DN set DockQty=@DNQty where pid=@DX
			update #INV set Qty=@DockQty-@DNQty,Remark=rtrim(Remark)+';('+convert(varchar(5),@iid)+')-->'+convert(varchar(5),@DNQty) where POVendor=@POVendor and MatNo=@IECPN and MaterialProperty='SHIP'
			end         
		end
    end		
    select @DX=@DX+1
...
(
--select distinct POVendor='IES',procument_part=left(procument_part,12),demand_part from DELL_r_shortage_for_real_demand_v200_all 
select distinct POVendor='ICC',procument_part=left(part_no,12),demand_part from ASUS_r_shortage_data where Demand_TYPE='FRU' /*len(rtrim(part_no))>12  and*/ and demand_part like '%FR_%'
) as a 

update #init set WU=replace(replace(replace(WU,'FR_',''),'NB_',''),'RD_','') 
...
------(2022/01/08)  not enough --> substring(WU,charindex(';',WU)+1,1000) to substring(WU,charindex(';',WU)+1,8000)
while @i<=@j
begin
     ...
     begin
         ...
         update #init set WU=substring(WU,charindex(';',WU)+1,8000) where iid=@i
     end
     select @i=@i+1 
end

----Keep LF /JF only
delete from #Mat_PF where (Pno like '1%' or Pno like '2%' or Pno like 'JC%' or Pno like 'LC%' or Pno like 'RC%')
---select * from #Mat_PF where Pno='LF2959AKB152'
...
(
--select distinct POVendor='IES',procument_part=left(procument_part,12),buyer,where_use=rtrim(where_use)+';' from DELL_r_shortage_for_real_demand_v200_all 
select distinct POVendor='ICC',procument_part=left(part_no,12),buyer=left(buyer,3),where_use from ASUS_r_shortage_data where  Demand_TYPE='FRU'/*len(rtrim(part_no))>12 and*/ and demand_part like '%FR_%'
) as a 

update #init2 set SA=replace(replace(replace(SA,'FR_',''),'NB_',''),'RD_','') 
...
begin
     ...
     begin
         ...
         update #init2 set SA=substring(SA,charindex(';',SA)+1,8000) where iid=@a
     end
     select @a=@a+1 
end
...
/*
delete from #Mat_PF where Material='6034B0047901'
delete from #Mat_SA where Material='6034B0047901'
*/
------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------
...
insert #mid131
select a.Material,a.mid,b.Material from #init131 a,t_download_org_bom_CP69DW b where a.mid=b.Component
...
insert #finalPF
   select * from #final131 where left(nextlevel,2) in ('LF','RF')
  
   
delete from #final131 where left(nextlevel,2) in ('LF','RF')
...
insert #finalPF 
select distinct * from #mid131 where left(nextlevel,2) in ('LF','RF')

delete from #mid131 where left(nextlevel,2) in ('LF','RF')
...
select distinct Material,mid,nextlevel,nextlevel from #finalPF where left(nextlevel,2) in ('LF','RF') and (Material like '13%' or Material like '11%')
) as a 
-------
) as a left join
...
select distinct ShortagePN from #result where RefPN=ShortagePN and ShortagePN like '13%') and left(RefPN,2) in ('LF','RF')) and RefPN=ShortagePN


----Insert the Material which parent is PF
insert #result
     select distinct POVendor,Pno,Pno,Material from #Mat_SA where (Material like '6%' or Material like '1%') and left(Pno,2) in ('LF','RF')
...
insert #initAssy
 select distinct POVendor,Pno,Pno from #Mat_SA where Material like '6%' and left (Pno,1)='1' and not left(Pno,2) in ('11','13','19','PF','LF','JF','RF') 
...
insert #midAssy
select POVendor,a.Assy,b.Material from #initAssy a,t_download_org_bom_CP69DW b where a.PF=b.Component and not (b.Material like 'LC%' or b.Material like 'JC%')

insert #finalAssy 
select distinct * from #midAssy where left(PF,2) in ('LF','RF')

--delete from #midAssy from #midAssy a,#finalAssy b where a.Assy=b.Assy
delete from #midAssy where left(PF,2) in ('LF','RF')
...
(select distinct * from #Mat_SA where Material like '6%' and left (Pno,1)='1' and not left(Pno,2) in ('11','13','19','LF','JF','RF')) as b
...
(
select distinct a.Material,a.Component from t_download_org_bom_CP69DW a,(
select b.Material,b.Alternative_item_group,b.Item_number from #result a,t_download_org_bom_CP69DW b where a.RefPN=b.Material and a.ShortagePN=b.Component) as b 
...
--select * from #result where RefPN='LFE032CMB002' and ShortagePN='6037B0065512'
...
--Remove Carton
delete from #result where ShortagePN like '606%' and not ShortagePN in (select distinct Material from t_download_matmas_CP69DW where Material_descript like 'LABEL,AGENCY%')
/*
drop table #AA
drop table #AAmid
drop table #AAResult
...
(select Material,Component,Alternative_item_group,Priority,Usage_probability,Item_number,Quantity from t_download_org_bom_CP69DW) as b where a.Component=b.Material and a.ShortagePN=b.Component
and a.AG=''
...
(select * from #AA where not AG in ('','NA')) as a,t_download_org_bom_CP69DW b where a.Component=b.Material and a.Item=b.Item_number 
...
from #AA a,t_download_org_bom_CP69DW b where a.Component=b.Material and not (b.Component like '6%' or b.Component like '171%')
--and a.PF  in ('LF3088DMB002','LFE033FMB002')
and Alternative_item_group is null --order by PF,ShortagePN,RefPN 
union
select distinct POVendor,PF,b.Component,a.RefPN,a.ShortagePN,'','','','',a.Qty--,b.Alternative_item_group,b.Priority,b.Usage_probability,b.Item_number
from #AA a,t_download_org_bom_CP69DW b where a.Component=b.Material and not  (b.Component like '6%' or b.Component like '171%')
--and a.PF  in ('LF3088DMB002','LFE033FMB002')
 and  not Alternative_item_group is null -- order by PF,ShortagePN,RefPN 
...
----Find Strange PF MB and update the Alternative
update #AAResult set AG='Y' ,Usage='50' where PF in (
select PF from (
select PF,RefPN,AG,Priority,Usage,Item,qty=count(*) from #AAResult  group by  PF,RefPN,AG,Priority,Usage,Item) as a where qty>1) and (PF like 'LF%MB%' or PF like 'RF%MB%')

----Strange data force to change 
/*
select * from #result where PF='LF3088DMB002'
select * from #AA where PF='LF3088DMB002'
select * from #AAResult where PF='LF3088DMB002'

select * from #result where PF='LFE033FMB002'
select * from #AA where PF='LFE033FMB002'
select * from #AAResult where PF='LFE033FMB002'

select * from #AAResult where PF='LFE050AKB002'
*/


--update #AAResult set ShortagePN='6037B0080212' where ShortagePN='6037B0081012'
--update #AAResult set ShortagePN='6037B0065612' where ShortagePN='6037B0065512'
--update #AAResult set Qty='0.001' where ShortagePN='6071B0010101'
...
begin   
update #FindQty set Qty=convert(int,convert(float,replace(replace(b.Quantity,' ',''),',',''))) from #FindQty a,t_download_org_bom_CP69DW b where a.SA=b.Material and a.ShortagePN=b.Component
and a.Qty=0 
...
insert #midFindQty
     select a.PF,b.Component,a.RefPN,a.ShortagePN,0 from #FindQty a,t_download_org_bom_CP69DW b where a.SA=b.Material and not Component like '6%'
...
--select * from #AAResult where PF='LF1930LMB002'

--select PF from (select PF,qty=count(*) from #AAResult where AG='NA' and ShortagePN like '13%' group by PF) as a where qty>1
--select * from #AAResult where AG='NA' and ShortagePN like '13%' order by PF
-----------------------------------------------------------------------------------------------
/*
...
-----(2021/08/05) 修正板階消失問題
-----(2021/07/21) 硬改---又有172代用又有代用
update #AAResult set Usage='30',Item='1000' where PF in (
select PF from  #AAResult where PF in (
select PF from (select PF,qty=count(*) from #AAResult where  ShortagePN like '13%' group by PF) as a where qty>1
--select PF from #AAResult where AG='NA' and ShortagePN like '13%'
)  and ShortagePN like '13%' and AG='NA') and AG='Y' and not ShortagePN like '6%'

update #AAResult set AG='Y',Usage='40' ,Item='1000'  where PF in (
select PF from  #AAResult where PF in (
select PF from (select PF,qty=count(*) from #AAResult where  ShortagePN like '13%' group by PF) as a where qty>1
--select PF from #AAResult where AG='NA' and ShortagePN like '13%'
)  and ShortagePN like '13%' and AG='NA')  and AG='NA' and not ShortagePN like '6%'
/*
select * from #AAResult where PF in (
select PF from  #AAResult where PF in (select PF from #AAResult where AG='NA' and ShortagePN like '13%')  and not AG='NA') order by PF
*/
--select * from #AAResult where PF='LFE032CMB002'
...
----(2015/10/12) change Qty to 1
update #AAResult set Qty=1 where PF in ('SF1087ALM002','SF1087BLM002','SF1087CLM002','SF1072ALM001','SF1071ALM001','SF1823ASL001','SF1823BSL001') 
and ShortagePN in ('6071B0008202','6071B0014202','6071B0014301')


--select * from #result where PF='LF1930LMB002'
--select * from #AAResult where PF='LF1930LMB002'
...
---------------------------------------------------------------------------------------------------------------------------------------------
	 if exists(select * from #INV where POVendor=@WPOVendor and MatNo=@WIECPN and MaterialProperty='FG' /*and left(MatNo,2) in ('PF','LF','JF')*/ and Qty>0)
	 begin --10
	 --while @WOpenQty>0
	 --		begin --12			    
				select top 1 @DetailQty=Qty,@INVDate=INVDate from #INV where POVendor=@WPOVendor and MatNo=@WIECPN and MaterialProperty='FG' /*and left(MatNo,2) in ('PF','LF','JF')*/ and Qty>0 order by INVDate
...
delete from OPS_OPO where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOR1 where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOR2 where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOSummary where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOSummary2 where Customer='ASUS' and ReportDate=convert(char(10),getdate(),111)---Do not Clear historical data .
delete from OPS_OM where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_Material where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_MSummaryPIC where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_MSummaryType where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_Alt where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_Inventory where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_RawOPO where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OTDFailD where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OTDFailS where Customer='ASUS' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
/*
select * from OPS_OPO where Customer='ASUS'
select * from OPS_OPOR1 where Customer='ASUS'
select * from OPS_OPOR2 where Customer='ASUS'
select * from OPS_OPOSummary where Customer='ASUS'
select * from OPS_OPOSummary2 where Customer='ASUS'---Do not Clear data .
select * from OPS_OM where Customer='ASUS'
select * from OPS_Material where Customer='ASUS'
select * from  OPS_MSummaryPIC where Customer='ASUS'
select * from  OPS_MSummaryType where Customer='ASUS'
select * from  OPS_Alt where Customer='ASUS'
select * from  OPS_Inventory where Customer='ASUS'
select * from  OPS_RawOPO where Customer='ASUS'
select * from  OPS_OTDFailD where Customer='ASUS'
select * from  OPS_OTDFailS where Customer='ASUS'
*/
...
(select POVendor=case when POVendor='CP81' then 'IES' when POVendor='CP69' then 'ICC' end,IECPN,M1=sum(M1) from FCST 
...
---OPO Detail
insert OPS_OPO
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),iid,Site,PIC,PO,SO,IECPO,POItem,CPQNo,IECPN,ProductFamily,
POVendor,POReceiveDate,PO_Type,Model_Status,MP,FCST_Status,PType,NeedShipDate,POQty,DNQty,
...
------OPOR Status--By Site
insert OPS_OPOR1
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),Site,PIC,OPOR,NPI_ItemQty=sum(NPI_ItemQty),Normal_ItemQty=sum(Normal_ItemQty) from 
(
...
------OPOR Status --By OPOR
insert OPS_OPOR2
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOR,NPI_ItemQty=sum(NPI_ItemQty),Normal_ItemQty=sum(Normal_ItemQty) from 
(
select Site,PIC,OPOR,NPI_ItemQty=sum(qty),Normal_ItemQty=0 from (
select Site,PIC,
OPOR=case 
...
select Customer='ASUS',* from #OPOSummary
...
from OPS_OPOSummary where Customer='ASUS' and ReportDate=convert(char(10),getdate(),111) and PType='6' group by Customer,ReportDate,PType 
union
...
NPINewItem=sum(NPInewItem),NPINewQty=sum(NPInewQty),NPINearFailItem=sum(NPInearfailItem),NPINearFailqty=sum(NPInearfailqty),
NPIFailItem=sum(NPIfailItem),NPIFailqty=sum(NPIfailqty)
from OPS_OPOSummary where Customer='ASUS' and ReportDate=convert(char(10),getdate(),111) and not PType='6' group by Customer,ReportDate,PType 
...
(
select a.*,PIC=case when a.ShortagePN like '13%' then 'Joan' when a.ShortagePN like '14%' then 'Joan' when a.ShortagePN like '15%' then 'Iris' else b.PIC end from 
(select a.*,b.Material_descript,b.Material_group from #AAResult a,t_download_matmas_CP69DW b where left(a.ShortagePN,12)=b.Material) as a left join
...
(select * from Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111) and Customer='ASUS') as b on a.POVendor=b.POVendor and a.ShortagePN=b.Material
order by iid

insert OPS_OM
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),* from #OM

--select * from #OM where PIC is null
...
insert #VD
select distinct a.Material,a.Priority,b.Vendor_Name2 from t_download_sourcelist_CP69DW a,dbo.t_download_supplier_master b where a.Vendor1=b.Vendor_Code and Material in (
...
NeedShipDate varchar(20),LatestSODate varchar(20),
LatestNeedShipDate varchar(20),Site varchar(20),Item varchar(20),RealOpenQty int,RestNeedQty int,RemainedStock int,Buyer varchar(30),Vendor varchar(500),
...
PIC=case when ShortagePN like '13%' then 'Joan' when ShortagePN like '15%' then 'Iris' else b.PIC end from 
...
(
select distinct * from Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111)
...
insert #MDCount
select distinct a.ShortagePN,b.Material_group from #AAResult a,t_download_matmas_CP69DW b where a.PF=b.Material and a.ShortagePN in (select distinct ShortagePN from #ShortageDetail)
...
(select a.PN,b.iid from #SMCount a,OPS_OM b where  Customer='ASUS' and ReportDate=convert(char(10),getdate(),111) and a.PN=b.ShortagePN ) as a left join
(select iid,MP,PO_Type from OPS_OPO where Customer='ASUS' and ReportDate=convert(char(10),getdate(),111)) as b on a.iid=b.iid
...
update #ShortageDetail_total set Material_descript='( '+isnull(b.Material_group,'')+' )    '+isnull(b.Material_descript,'') from #ShortageDetail_total a,t_download_matmas_CP69DW b 
...
update #ShortageDetail_total set PIC=c.PIC from #ShortageDetail_total a,t_download_matmas_CP69DW b,MType c where replace(a.ShortagePN,'*','')=b.Material and b.Material_group=c.Material_group
and a.PIC=''


-----(2022/10/04) update all PIC to 'Var'
update #ShortageDetail_total set PIC='Var' --where ShortagePN like '13%' and PIC=''
...
(select distinct * from Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111)) as b
...
where a.POVendor=b.POVendor and a.ShortagePN=b.MatNo and ETADiff<0
...
where a.POVendor=b.POVendor and a.ShortagePN=b.MatNo and ETADiff>0 and not ETADiff=9999
...
and a.ShortagePN=b.MatNo and b.InventoryType='MT-WH' and b.MP='M_FG'
...
and a.ShortagePN=b.MatNo and b.InventoryType='M-WH' and b.MP='M_FG'

update #ShortageDetail_total set MStock=b.Qty  from #ShortageDetail_total a,Ivan_Current_M_INV b where a.POVendor=b.POVendor 
and a.ShortagePN=b.MatNo and b.MP='M_FG' and a.ShortagePN like '13%'


/*
-------(2020/05/15) Temporary add...
update #ShortageDetail_total set MStock=b.Qty from #ShortageDetail_total a,
(select Material,Qty=sum(convert(float,replace(replace(Qty,',',''),' ',''))) from ASUS_t_download_inv where Material like '13%' group by Material) as b
where  a.ShortagePN=b.Material
*/

insert OPS_Material
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),*  from #ShortageDetail_total order by PIC,NeedShipDate,RealOpenQty,GattingNeed desc


-----------Summary (By PIC)-----------------------------------------------------------------
insert OPS_MSummaryPIC
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),PIC=isnull(PIC,''),Item=sum(Item),Qty=sum(Qty),NoETAItem=sum(NoETAItem),NoETAQty=sum(NoETAQty) from
(
...
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),MType,PIC=isnull(PIC,''),Item=sum(Item),Qty=sum(Qty),NoETAItem=sum(NoETAItem),NoETAQty=sum(NoETAQty) from
(
...
---Alternative
insert OPS_Alt
select distinct Customer='ASUS',ReportDate=convert(char(10),getdate(),111),MS,ShortagePN,Priority,Usage,Item,RefPN from #Alternative order by MS,RefPN,Item

---Shortage material Inventory consumer status
insert OPS_Inventory
select distinct Customer='ASUS',ReportDate=convert(char(10),getdate(),111),a.POVendor,a.MaterialProperty,a.MatNo,Material_descript='( '+isnull(b.Material_group,'')+' )    '+isnull(b.Material_descript,''),
RemainedQty=Qty,StockExistedDate=INVDate,ConsumerStatus=a.Remark from #INV a,t_download_matmas_CP69DW b where left(a.MatNo,12)=b.Material 


insert OPS_RawOPO
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),a.*,MaterialETA=isnull(MaterialETA,''),Remark=isnull(Remark,'') from
(
select * from (
...
Open_Qty=convert(float,replace(replace(Open_Qty,' ',''),',','')) 
from cp69_t_download_nb_po where Plnt='CP69' and S_loc='SWA3' 
--union
--select PO_Number,Item,Plnt,Material,Description,Vendor,Buyer,Create_Dt=convert(char(10),convert(datetime,Create_Dt+' 00:00'),111),
--PO_Qty=convert(float,replace(PO_Qty,' ','')),
--Open_Qty=convert(float,replace(Open_Qty,' ','')) 
--from icc_t_download_nb_po where Plnt='CP62' and S_loc='SW03'
) as a 
) as a left join
(select distinct POVendor=case when POVendor='ICC' then 'CP69' else '' end,Material,MaterialETA,Remark 
from dbo.Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111) and Customer='ASUS'
...
------Normal OTD
insert OPS_OTDFailD
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='Normal OTD',* from (
...
---NPI OTD 
insert OPS_OTDFailD
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='NPI OTD',* from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Normal Potential
insert OPS_OTDFailD
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='Normal Potential',* from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
----NPI Potential
insert OPS_OTDFailD
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='NPI Potential',* from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Summary Normal OTD 
insert OPS_OTDFailS
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='Normal OTD',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Summary NPI OTD
insert OPS_OTDFailS
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='NPI OTD',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Summary
insert OPS_OTDFailS
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='Normal Potential',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
insert OPS_OTDFailS
select Customer='ASUS',ReportDate=convert(char(10),getdate(),111),OPOType='NPI Potential',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
(select IECPN,ProductFamily,PO,POItem,NeedShipDate=min(NeedShipDate),OpenQty=sum(OpenQty) from #OPODetail 
