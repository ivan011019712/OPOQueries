...
delete from OldOPO where ReportDate=convert(char(10),getdate(),111) and Customer='HP_ITH'
delete from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111) and Customer='HP_ITH' 

select *,EST_Date='0000/00/00',IES_DN='--------------------',IES_DNPGI='0000/00/00',MType='--------------------',Category='-----------',LateThanNeedShipDate='-',OTDType='----------',OTDStatus='----------', 
Result='----------------------------------------' into #tmp from OPS_OPO where ReportDate=convert(char(10),getdate(),111) and Customer='HP_ITH'
...
---Detail
delete from OPS_OPO_new where ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111) and Customer='HP_ITH'
delete from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111) and Customer='HP_ITH'
...
select iid from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new) and Shortage='Y') and Customer='HP_ITH' and ReportDate=(select max(ReportDate) from OPS_OPO_new)
...
(select * from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new) and Customer='HP_ITH') as a left join
...
else 'Others' end,DNQty,IES_DNPGI from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111)  and Customer='HP_ITH' and not IES_DN='' and DNQty>0
