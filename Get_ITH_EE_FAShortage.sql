...
delete from OPS_BL where ReportDate=convert(char(10),getdate(),111) and Customer='HP_ITH'
drop table #BLD
*/
update ITH_BLMaterial$ set Remark=substring(Remark,charindex(' )',Remark)+3,1000) where Remark like '%OPO -->%'
...
select MType='NB',Material=left(part_no,12),description,buyer,demand_part,ManufactureStock=stock,ServiceStock=fstock,ManufactureExcessStock=jstock,TrialStock=pstock,SHStock=bstock,Intransit=instransit,ShortageQty=total_qty from ith_r_shortage_data 
...
update #tmp set NBStock=b.Qty from #tmp a,Ivan_CurrentINV b where a.Material=b.MatNo and b.Customer='HP_ITH' and INVDate=convert(char(10),getdate(),111)
...
select * from OPS_OPO_new where Customer='HP_ITH' and ReportDate=convert(char(10),getdate(),111)
...
(select Material,b.PIC from t_download_matmas_TH02DW a,MType b where a.Material_group=b.Material_group and Material in (select distinct Material from #result)) as b
where a.Material=b.Material 


update #result set New='' from #result a,ITH_BLMaterial$ b where a.MType=b.MType and a.Material=b.Material

-----(2017/11/29) Add Vmi Stock
update #result set VmiStock=b.VmiStock from #result a,
(select part_no,VmiStock=sum(available_qty) from t_download_vmi_852 where plant='TH02' group by part_no) as b 
...
---Update Inventory
update #sub set INV=b.Qty from #sub a,Ivan_CurrentINV b where a.Component=b.MatNo and b.Customer='HP_ITH' and INVDate=convert(char(10),getdate(),111)
...
--select * from (
select distinct a.*,[Material ETA]=isnull(b.[Material ETA],'_'),Remark=isnull(b.Remark,'_') into #BL from #result as a left join (select distinct * from ITH_BLMaterial$) as b on a.MType=b.MType and a.Material=b.Material order by AbleToTransfer desc,description
...
----------------------------------------------
----------------------------------------------
delete from OPS_BL where Customer='HP_ITH' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))

insert OPS_BL 
   select convert(char(10),getdate(),111),'HP_ITH',* from #BL 
...
(select Material,description,[Material ETA],Remark from OPS_BL where Customer='HP_ITH' and ReportDate=convert(char(10),getdate(),111)) as b
...
(select IECPN,NeedShipDate=min(NeedShipDate),OpenQty=sum(OpenQty) from OPS_OPO_new where Customer='HP_ITH' and ReportDate=convert(char(10),getdate(),111)
...
-----(2024/10/23) Add SW01 015 Stock
	...
	(
		...
		(
			...
			(select * from OPS_BL where Customer='HP_ITH' and ReportDate=convert(char(10),getdate(),111) ) as a left join 
			(select * from Ivan_qi where ReportDate=convert(char(10),getdate(),111) and Customer='ITH_BL') as b on  a.Material=b.ShortagePN --order by PIC,Priority,eMsg

			) as a left join
-----(2026/03/25) Add plant to seperate the ICC & ITH
			 (select MatNo,Qty from WIP_WHALL_TD where SLoc='WA1' and SType='015' and Plant in ('TH02','TH05','TH60'))  as b on a.Material=b.MatNo --order by PIC,Priority,eMsg

	 		) as a left join

		 (select MatNo,Qty from WIP_WHALL_TD where SLoc='WA1' and SType='005'  and Plant in ('TH02','TH05','TH60')) as b on a.Material=b.MatNo --order by PIC,Priority,eMsg

	) as a left join
	(
	select Material,Qty from
	(select * from OMS$ where StockDate=(select max(StockDate) from OMS$)) as a, t_download_matmas_TH02DW as b where a.HPPN=Old_Material
