...
insert #INV
    select POVendor,MP,MatNo,Qty,INVDate,'' from Ivan_CurrentINV  where Customer='H2' and INVDate>=convert(char(10),getdate(),111) 
...
--Update Escalation
update #init_tmp_OPO set Escalation=b.Escalation,EscalationDate=b.[Escalation Date],Remark=b.Remark from #init_tmp_OPO a,H2_OPO$ b where a.Site=b.Site and a.SO=b.SO and a.IECPN=b.IECPN
...
insert #tmp_OPO
   select distinct * from #init_tmp_OPO where PO_Type in ('NPIH','ALC')  and POVendor='ICC' order by NeedShipDate,POReceiveDate         
...
/*
delete from #Mat_SA where Material='6037B0284805'
delete from #Mat_SA where Material='6037B0284808'
delete from #Mat_SA where Material='6037B0284815'
delete from #Mat_SA where Material='6037B0284813'
delete from #Mat_SA where Material='6037B0284816'
delete from #Mat_SA where Material='6037B0284815'


delete from #Mat_PF where Material='6037B0284805'
delete from #Mat_PF where Material='6037B0284808'
delete from #Mat_PF where Material='6037B0284815'
delete from #Mat_PF where Material='6037B0284813'
delete from #Mat_PF where Material='6037B0284816'
delete from #Mat_PF where Material='6037B0284815'
...
delete from OPS_WHFinal where Customer='H2'
insert OPS_WHFinal
       select 'H2',*,getdate() from #WHFinal     
/*
---------------------------------------------------New Report -----------------------------------------------------
delete from OPS_INV
...
delete from OPS_OPO where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOR1 where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOR2 where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOSummary where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OPOSummary2 where Customer='H2' and ReportDate=convert(char(10),getdate(),111)---Do not Clear historical data .
delete from OPS_OM where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_Material where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_MSummaryPIC where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_MSummaryType where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_Alt where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_Inventory where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_RawOPO where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OTDFailD where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
delete from OPS_OTDFailS where Customer='H2' and (ReportDate=convert(char(10),getdate(),111) or ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111))
/*
...
---OPO Detail
insert OPS_OPO
select Customer='H2',ReportDate=convert(char(10),getdate(),111),iid,Site,PIC,PO,SO,IECPO,POItem,CPQNo,IECPN,ProductFamily,
POVendor,POReceiveDate,PO_Type,Model_Status,MP,FCST_Status,PType,NeedShipDate,POQty,DNQty,
...
------OPOR Status--By Site
insert OPS_OPOR1
select Customer='H2',ReportDate=convert(char(10),getdate(),111),Site,PIC,OPOR,NPI_ItemQty=sum(NPI_ItemQty),Normal_ItemQty=sum(Normal_ItemQty) from 
(
...
------OPOR Status --By OPOR
insert OPS_OPOR2
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOR,NPI_ItemQty=sum(NPI_ItemQty),Normal_ItemQty=sum(Normal_ItemQty) from 
(
select Site,PIC,OPOR,NPI_ItemQty=sum(qty),Normal_ItemQty=0 from (
select Site,PIC,
OPOR=case 
...
select Customer='H2',* from #OPOSummary
...
from OPS_OPOSummary where Customer='H2' and ReportDate=convert(char(10),getdate(),111) and PType='6' group by Customer,ReportDate,PType 
union
...
NPINewItem=sum(NPInewItem),NPINewQty=sum(NPInewQty),NPINearFailItem=sum(NPInearfailItem),NPINearFailqty=sum(NPInearfailqty),
NPIFailItem=sum(NPIfailItem),NPIFailqty=sum(NPIfailqty)
from OPS_OPOSummary where Customer='H2' and ReportDate=convert(char(10),getdate(),111) and not PType='6' group by Customer,ReportDate,PType 
...
union
*/
select a.*,b.Material_descript,b.Material_group from #AAResult a,t_download_matmas_CP60DW b where left(a.ShortagePN,12)=b.Material
) as a
...
(select * from Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111) and Customer='H2') as b on a.POVendor=b.POVendor and a.ShortagePN=b.Material
order by iid
...
insert OPS_OM
select Customer='H2',ReportDate=convert(char(10),getdate(),111),* from #OM
...
(
select distinct * from Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111) and Customer='H2'
...
(select a.PN,b.iid from #SMCount a,OPS_OM b where  Customer='H2' and ReportDate=convert(char(10),getdate(),111) and a.PN=b.ShortagePN ) as a left join
(select iid,MP,PO_Type from OPS_OPO where Customer='H2' and ReportDate=convert(char(10),getdate(),111)) as b on a.iid=b.iid
...
(select distinct * from Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111) and Customer='H2') as b
...
where a.POVendor=b.POVendor and a.ShortagePN=b.MatNo and ETADiff<0 and Customer='H2'
...
where a.POVendor=b.POVendor and a.ShortagePN=b.MatNo and ETADiff>0 and not ETADiff=9999 and Customer='H2'
...
and a.ShortagePN=b.MatNo and b.InventoryType='MT-WH' and b.MP='M_FG' and Customer='H2'
...
and a.ShortagePN=b.MatNo and b.InventoryType='M-WH' and b.MP='M_FG' and Customer='H2'
*/
...
(select * from Ivan_Current_M_INV where MP='MT-WH' and Customer='H2') as b on a.SP=b.MatNo and a.POVendor=b.POVendor
...
(select * from Ivan_Current_M_INV where MP='M_FG' and Customer='H2') as b on a.SP=b.MatNo and a.POVendor=b.POVendor
) as a where not Qty is null group by POVendor,ShortagePN
) as b where a.POVendor=b.POVendor and a.ShortagePN=b.ShortagePN
...
insert OPS_Material
select Customer='H2',ReportDate=convert(char(10),getdate(),111),*  from #ShortageDetail_total order by PIC,NeedShipDate,RealOpenQty,GattingNeed desc



-----------Summary (By PIC)-----------------------------------------------------------------
insert OPS_MSummaryPIC
select Customer='H2',ReportDate=convert(char(10),getdate(),111),PIC=isnull(PIC,''),Item=sum(Item),Qty=sum(Qty),NoETAItem=sum(NoETAItem),NoETAQty=sum(NoETAQty) from
(
...
select Customer='H2',ReportDate=convert(char(10),getdate(),111),MType,PIC=isnull(PIC,''),Item=sum(Item),Qty=sum(Qty),NoETAItem=sum(NoETAItem),NoETAQty=sum(NoETAQty) from
(
...
---Alternative
insert OPS_Alt
select distinct Customer='H2',ReportDate=convert(char(10),getdate(),111),MS,ShortagePN,Priority,Usage,Item,RefPN  from #Alternative order by MS,RefPN,Item

---Shortage material Inventory consumer status

insert OPS_Inventory
select distinct Customer='H2',ReportDate=convert(char(10),getdate(),111),a.POVendor,a.MaterialProperty,a.MatNo,Material_descript='( '+isnull(b.Material_group,'')+' )    '+isnull(b.Material_descript,''),
RemainedQty=Qty,StockExistedDate=INVDate,ConsumerStatus=a.Remark from #INV a,t_download_matmas_CP81DW b where left(a.MatNo,12)=b.Material and POVendor='IES'


insert OPS_Inventory
select distinct Customer='H2',ReportDate=convert(char(10),getdate(),111),a.POVendor,a.MaterialProperty,a.MatNo,Material_descript='( '+isnull(b.Material_group,'')+' )    '+isnull(b.Material_descript,''),
RemainedQty=Qty,StockExistedDate=INVDate,ConsumerStatus=a.Remark from #INV a,t_download_matmas_CP60DW b where left(a.MatNo,12)=b.Material and POVendor='ICC'


---(2019/02/14) Remove SW27 
---(2013/07/01) Raw Material OPOs 
insert OPS_RawOPO
select Customer='H2',ReportDate=convert(char(10),getdate(),111),a.*,MaterialETA=isnull(MaterialETA,''),Remark=isnull(Remark,'') from
(
...
from dbo.Ivan_ShortageMaterialUpdate where convert(char(10),Cdt,111)=convert(char(10),getdate(),111) and Customer='H2'
...
and Customer='H2' and ReportDate=convert(char(10),getdate(),111)
...
------Normal OTD
insert OPS_OTDFailD
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='Normal OTD',* from (
...
---NPI OTD 
insert OPS_OTDFailD
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='NPI OTD',* from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Normal Potential
insert OPS_OTDFailD
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='Normal Potential',* from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
----NPI Potential
insert OPS_OTDFailD
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='NPI Potential',* from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Summary Normal OTD 
insert OPS_OTDFailS
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='Normal OTD',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Summary NPI OTD
insert OPS_OTDFailS
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='NPI OTD',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
---Summary
insert OPS_OTDFailS
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='Normal Potential',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
...
insert OPS_OTDFailS
select Customer='H2',ReportDate=convert(char(10),getdate(),111),OPOType='NPI Potential',MType=left(Material_descript,charindex(')',Material_descript)),PType=count(distinct ShortagePN),ItemQty=count(POItem),PTypeQty=sum(OpenQty) from (
select * from (
select a.*,b.ShortagePN,b.Material_descript,b.MaterialETA,b.Remark from
(select IECPN,ProductFamily,PO,POItem,NeedShipDate=min(NeedShipDate),OpenQty=sum(OpenQty) from #OPODetail 
