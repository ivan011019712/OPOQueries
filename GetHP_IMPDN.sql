...
delete from OldOPO where ReportDate=convert(char(10),getdate(),111) and Customer='HP_IMP'
delete from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111) and Customer='HP_IMP' 

select *,EST_Date='0000/00/00',IES_DN='--------------------',IES_DNPGI='0000/00/00',MType='--------------------',Category='-----------',LateThanNeedShipDate='-',OTDType='----------',OTDStatus='----------', 
Result='----------------------------------------' into #tmp from OPS_OPO where ReportDate=convert(char(10),getdate(),111) and Customer='HP_IMP'
...
) as b where a.SO=b.SO and a.IECPN=b.IECPN and a.POReceiveDate=b.PO_Date and a.POItem=b.SOItem--b.Item

------(2025/11/19) Get DN by SO Item
update #tmp set IES_DN=b.IES_DN from #tmp a,
(select distinct a.*,newItem=Item/*convert(int,b.IPCSOItem)*/  from #DN a,ZM57 b where a.SO=b.IECSO and a.Item=b.IECSPItem) as b
...
---Detail
delete from OPS_OPO_new where ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111) and Customer='HP_IMP'
delete from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111) and Customer='HP_IMP'
...
select iid from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new) and Shortage='Y') and Customer='HP_IMP' and ReportDate=(select max(ReportDate) from OPS_OPO_new)
...
(select * from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new) and Customer='HP_IMP') as a left join
...
else 'Others' end,DNQty,IES_DNPGI from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111)  and Customer='HP_IMP' and not IES_DN='' and DNQty>0
