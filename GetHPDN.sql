...
delete from OldOPO where ReportDate=convert(char(10),getdate(),111) and Customer='HP'
delete from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111) and Customer='HP'

select *,EST_Date='0000/00/00',IES_DN='--------------------',IES_DNPGI='0000/00/00',MType='--------------------',Category='-----------',LateThanNeedShipDate='-',OTDType='----------',OTDStatus='----------', 
Result='----------------------------------------' into #tmp from OPS_OPO where ReportDate=convert(char(10),getdate(),111) and Customer='HP'
...
---Add EST_Date
---2021/04/14,1. (DN exists) If exist DN ,and equal Open Qty then EST_Date folows DN Date .
update #tmp set EST_Date=IES_DNPGI where not IES_DNPGI='' and EST_Date='0000/00/00' and OpenQty=DNQty
...
update #tmp set EST_Date=convert(char(10),dateadd(dd,14,convert(datetime,FutureFirstSupportDate)),111) where not FutureFirstSupportDate='' and EST_Date='0000/00/00' and substring(IECPN,8,2)='MB'

----(2022/07/21) Add more models
----(2022/07/14) all K/B change to 14 days 
----(2022/07/07) new K/B change to 14 days 
...
and ((substring(IECPN,3,4) in ('BR01','BS01','BU01','CE01','CH31','CJ01','CS01','CS21','DG01','DS01','DS21',
'DS51','DT01','DT51','DU01','EE01','EF01','EF51','EF61','EF71','FL01','FL71',
'FP01','FS71','FW71','FX01','FY76','GB31','GB51','GC31','GC71','GD01','GE71',
'GG01','GG61','GH01','GH71','GJ01','GJ31','GK31','GK71','GM01','GP71','GQ31',
'GQ71','GR01','GS71','GS72','GU01','GU02','GU03','GW31','GY01','GZ31','IH71',
'IK71','PM01','FC02','FC71','FC26','FE01','FC41','FE02','FC61','FR01','FC62','FR31',
'FR01','FU41','  FW01','FZ01','FZ31','FZ71','GX71','GX31','GQ01',
'DC31','II01','IH31','IH01','EF02','CP01','DF01','CU03','CU06','CU07',
'DA01','DA02','DW01','FU01','FC42','EA01','EA03','EA04','FF01','DQ01',
'GY31','FW01','FX71','GJ71','ED01','CI01','CU31','CU33','GM05','GG31',
'IF01','IF31','IF71','FD62','IB71','DJ01','FY01','EG01','II31','IE01',
'FP31','GC01','FM01','2850','IG71','DY01','DV21','DC01','DC02','IL32',
'IM01','GY02','IK01','IE72','IC31','IC71','ID01','ID31','FT01')
or substring(IECPN,3,2) in ('GH','GB','DX','FT','FK','EK','GA','GO')) and substring(IECPN,8,2)='KB' --and POReceiveDate>'2022/07/06')
or substring(IECPN,3,4) in (select distinct substring(IECPN,3,4) from EOLInfo$ where FCS_Dt>='2022/07/01' and substring(IECPN,8,2)='KB'))
...
---Detail
delete from OPS_OPO_new where ReportDate<=convert(char(10),dateadd(dd,-90,getdate()),111) and Customer='HP'
delete from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111) and Customer='HP'
...
select iid from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new) and Shortage='Y') and Customer='HP' and ReportDate=(select max(ReportDate) from OPS_OPO_new)
...
(select * from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new) and Customer='HP') as a left join
...
--update #OPOresult set PO=rtrim(PO)+'_(23Q1)' where PO in ('15628369')
...
---(2025/05/18) add AMS
update #OPOresult set PO=rtrim(a.PO)+'_AMS' from #OPOresult a,SMSBP b where a.PO=b.PO and a.CPQNo=b.OSSPPN and b.PT='AMS'

---(2025/05/04) add VDS
update #OPOresult set PO=rtrim(a.PO)+'_VDS' from #OPOresult a,SMSBP b where a.PO=b.PO and a.CPQNo=b.OSSPPN and b.PT='VDS'
...
else 'Others' end,DNQty,IES_DNPGI from OPS_OPO_new where ReportDate=convert(char(10),getdate(),111)  and Customer='HP' and not IES_DN='' and DNQty>0
) as a group by PT,IES_DNPGI


/*Get "3G" DN & Qty
select IES_DNPGI,IECPN,sum(DNQty) from OPS_OPO_new where Customer='HP' and ReportDate='2017/03/24' --and PO like '%3G%' 
/*and Site='HP-FCH2'*/ and DNQty>0 group by IES_DNPGI,IECPN order by IES_DNPGI
*/

------------------------------------------------------------------------
------------------------------------------------------------------------
----------HP Daily OPO Analysis and OPO Detail End....
------------------------------------------------------------------------
------------------------------------------------------------------------

/*-----------------------------------------------------
------------Get Compare list with 5/4
--drop table #Compare

select [0504Result]=a.Result,ImproveDate=convert(char(10),a.ImproveDate,111),TargetDate=convert(char(10),a.TargetDate,111),a.Site
,a.PO,a.IECPN,a.ProductFamily,a.POReceiveDate,a.NeedShipDate,
a.POQty,b.DNQty,b.DockQty,b.ShipQty,b.OpenQty,b.Shortage,b.Remark,
b.EST_Date,b.IES_DNPGI,UpToDateResult=b.Result,b.MaterialRemark,a.Comment,a.Action,a.ActionPIC
 into #Compare from
(select a.*,b.ImproveDate,b.TargetDate,b.Comment,b.Action,ActionPIC=b.PIC from OPS_OPO_new a,OPO0511$ b where a.PO=b.PO and a.IECPN=b.IECPN and Customer='HP' and ReportDate='2016/05/04') as a left join
(
select a.*,MaterialRemark=isnull(b.Remark,'') from
(select * from OPS_OPO_new where ReportDate=(select max(ReportDate) from OPS_OPO_new)) as a left join
(select * from #OMFinal) as b on a.iid=b.iid 
) as b on a.PO=b.PO and a.SO=b.SO and a.IECPN=b.IECPN and a.POReceiveDate=b.POReceiveDate
and a.POQty=b.POQty


update #Compare set Comment='Shipped ( '+b.PndGIDate+ ' )' from #Compare a,
(select PO,SO,IECPN,PndGIDate=max(PndGIDate) from Service_APD where PndGIDate>'2016/04/01' group by PO,SO,IECPN) as b where a.PO=b.PO and a.IECPN=b.IECPN and OpenQty is null
...
---Sumary
select * from (
select FType='Closed',PType='',Item=count(*) from #Compare where Comment like '%Shipped%' or UpToDateResult like '%Dock%' 
union
select 'Delay',substring(IECPN,8,2),count(*) from #Compare where Comment like '(Delay%' and not UpToDateResult like '%Dock%' group by substring(IECPN,8,2)
union
select 'Help',substring(IECPN,8,2),count(*) from #Compare where Comment like '(Help%' and not UpToDateResult like '%Dock%' group by substring(IECPN,8,2)
union
select 'Risk',substring(IECPN,8,2),count(*) from #Compare where Comment like '(Risk%' and not UpToDateResult like '%Dock%' group by substring(IECPN,8,2)
union
select 'Material OK',PType='',count(*) from #Compare where Comment like '%OK%' and not UpToDateResult like '%Dock%' 
union
select 'Cancel',substring(IECPN,8,2),count(*) from #Compare where Comment like '%Cancel%' and UpToDateResult is null group by substring(IECPN,8,2)
) as a order by FType,Item desc,PType
